...
MNN_PUBLIC std::vector<VARP> _MaxPoolIndices(VARP x, INTS kernel, INTS stride = {1, 1}, PaddingMode pad = VALID, INTS pads= {0, 0}, bool Indices = false);
...
MNN_PUBLIC VARP _SliceConst(VARP x, const std::vector<int>& starts, const std::vector<int>& sizes) ;
...
MNN_PUBLIC VARP _Bucketize(VARP input, std::vector<float>& boundaries, int dtype);

MNN_PUBLIC VARP _CumSum(VARP x, VARP axis, bool exclusive, bool reverse);

MNN_PUBLIC VARP _HardSigmoid(VARP x, float alpha = 0.2f, float beta = 0.5f);
} // namespace Express
} // namespace MNN
