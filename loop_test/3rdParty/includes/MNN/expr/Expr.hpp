...
public:
    ...
    static EXPRP create(const OpT* op, std::vector<VARP> inputs, std::vector<std::string> outputNames, int outputSize = 1);
    ...
    static void repalcemTo(EXPRP oldExpr, EXPRP newExpr){
        oldExpr->mTo = newExpr->mTo;
    }
