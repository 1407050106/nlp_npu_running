...
struct ScheduleConfigV2 {
    ...
    /** hybrid compute, which layer op should be special precision 
     * Default: algin with backend
     * FP32: fp32 compute
     * FP16: fp16 compute
     * INT8: int8 compute
    */
    enum computePrecisionMode {Default = 0, FP32 = 1, FP16 = 2, INT8 = 3, INT4 = 4};
    std::map<std::string, computePrecisionMode> customLayers;
};
...
public:
    ...
    static Interpreter* createFromMtnnFile(const char* file);
    /**
    ...
    static Interpreter* createFromMtnnBuffer(const void* buffer, size_t size);
    ~Interpreter();
...
public:
    ...
    /**
     * @brief create session with schedule config v2. created session will be managed in net.
     * @param config session schedule config v2.
     * @return created session if success, NULL otherwise.
     */
    Session* createSession(const ScheduleConfigV2& config);
...
public:
     ...
     * @param session   given session.
     * @param infoType  runtime info type: MEMORY / CPU / FLOPS.
     * @param ptr     given info ptr, see SessionInfoCode for detail
     * @return true if support the infoType, false otherwise.
     */
    bool getSessionInfo(const Session* session, std::string& infoType, void* ptr);
